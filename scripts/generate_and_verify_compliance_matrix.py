...
def main():
    parser = argparse.ArgumentParser(description="Canonical Phase Compliance Matrix Generator & Verifier")
    parser.add_argument(
        "--write",
        action="store_true",
        help="Write and synchronize compliance matrix files under evidence/phase_compliance/ (milestone publication mode)."
    )
    parser.add_argument(
        "--check",
        action="store_true",
        help="Verify compliance matrix in-memory without disk modifications (default behavior)."
    )
    ...
    # Group requirements by phase in memory
    phases = {}
    ...
    for p in sorted(phases.keys()):
        all_matrix_reqs.extend(phases[p])
    ...
    if args.write:
        # Write mode: intentionally synchronize JSON and Markdown evidence on disk
        ...
        print(f"[WRITE MODE] PASS: Updated {final_md_file} and JSON matrices successfully.")
    else:
        print("[CHECK MODE] PASS: Compliance matrix verification passed in-memory (zero disk mutations).")
