...
object TransactionTypeNormalizer {
    ...
    private val RECOGNIZED_CANONICAL_TYPES = setOf("took", "gave", "renewal", "note")

    /**
     * Returns true if [canonicalType] is a known canonical type ("took", "gave", "renewal", "note").
     */
    fun isRecognizedCanonicalType(canonicalType: String): Boolean {
        return canonicalType in RECOGNIZED_CANONICAL_TYPES
    }
    ...
    fun isRecognizedType(rawType: String?): Boolean {
        ...
        return isRecognizedCanonicalType(canonical)
    }
}
