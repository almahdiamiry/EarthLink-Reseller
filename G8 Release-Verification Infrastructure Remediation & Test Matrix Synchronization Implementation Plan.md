...
>
> The current planning/authoring session may occur in a Windows environment. This is **not** the authoritative certification execution environment.
>
> The actual G8 remediation implementation and certification execution shall be performed in the **approved Linux environment**.
>
> Therefore:
> 1. **Linux is the authoritative execution environment** for all implementation verification, Gradle execution, G8 certification, release verification, and final `PRODUCTION_READY` determination.
> 2. **Windows-specific observations** from the planning session are diagnostic only and MUST NOT be treated as final certification evidence.
> 3. **Platform-specific commands, paths, shell syntax, process behavior, Java/Gradle behavior, Android SDK resolution, and signing-tool availability** MUST be validated in the Linux execution environment before being considered authoritative.
> 4. The implementation plan MUST NOT hard-code Windows-specific paths, environment variables, PowerShell behavior, or Windows-only tooling assumptions into G8 verification semantics.
> 5. The Linux environment MUST use the repository-authoritative Gradle wrapper and the canonical `scripts/run_verified_command.py` process boundary.
> 6. Any environment-dependent requirement that cannot be established in Linux MUST result in `FAIL-CLOSED / ENVIRONMENT-BLOCKED`, not a substituted Windows result.
> 7. Historical Windows diagnostic execution MUST NOT be promoted into current certification evidence.
> 8. Final evidence MUST record the authoritative Linux execution environment identity sufficient to reproduce and audit the certification boundary.

### Platform Execution Rule

The plan expresses commands in platform-neutral logical terms first, then resolves the actual Linux invocation during execution.

For the authoritative certification environment, the expected Gradle invocation is:
```bash
python3 scripts/run_verified_command.py ./gradlew <task>
```

Examples:
```bash
python3 scripts/run_verified_command.py ./gradlew testDebugUnitTest
python3 scripts/run_verified_command.py ./gradlew assembleRelease
```

The exact instrumentation task, if contract-applicable, MUST be discovered from the repository's current Android/Gradle configuration and executed through the same verified runner. It MUST NOT be guessed from Windows behavior or copied from an unverified historical environment.

### Environment Boundary Rule

```text
CURRENT WINDOWS SESSION
    = planning / inspection / diagnostic evidence only

APPROVED LINUX ENVIRONMENT
    = authoritative implementation + runtime + artifact + G8 certification evidence
```

`PRODUCTION_READY` MUST NOT be derived from Windows execution evidence when the authoritative certification boundary is Linux.
...
### Task 10: Fresh Current-HEAD Runtime Execution (Authoritative Linux Environment)

**Files:** None (Execution only).

> **Execution Boundary:** Execution MUST occur in the approved Linux certification environment. Do not substitute Windows execution results for Linux evidence.

- [ ] **Step 1: Resolve required corpora and execute test run via `run_verified_command.py` in Linux**
...
- **10C:** Execute instrumentation corpus ONLY when Task 1 applicability establishes it as mandatory for release gating:
  - If `contract.corpora_applicability.INSTRUMENTED_CORPUS.execution_task is not None`:
    Execute: `python3 scripts/run_verified_command.py ./gradlew {contract.corpora_applicability.INSTRUMENTED_CORPUS.execution_task}`
  - Else:
...
- [ ] **Step 1: Build release APK and verify complete AND-chain in Linux**
...
- [ ] **Step 1: Inspect actual CLI contract and execute certification pipeline in Linux**
...
2. Execute certification engine:
   `python3 scripts/run_verified_command.py python3 scripts/g8_certify.py`
3. Execute independent verifier using the exact discovered CLI interface:
   `python3 scripts/run_verified_command.py python3 scripts/g8_verify_certification_bundle.py build/g8_certification/g8_certification_bundle.json`
...
Verify all automated gates pass unconditionally in the Linux execution boundary:
```bash
python3 scripts/run_verified_command.py python3 -m pytest tests/g8/ -v
python3 scripts/run_verified_command.py python3 scripts/verify_test_environment_matrix.py
python3 scripts/run_verified_command.py python3 scripts/verify_invariant_contract.py
python3 scripts/run_verified_command.py python3 scripts/scan_forbidden_patterns.py
```
...
| **GATE-09: Runtime & Artifact Path** | Machine | All mandatory runtime corpora executed in approved Linux environment + release artifact independently verified + final verifier executed in same boundary | Verified execution of mandatory corpora in Linux + `apksigner verify` + `g8_verify_certification_bundle.py` | REQUIRED |
...
12. All mandatory exit gates are machine-verifiable in the authoritative Linux environment.
...
- [ ] Release APK SHA-256 is independently recomputed from binary and bound to the certification boundary.
